...
extended_bane_enfeeble       = extended_bane_enfeeble       or class({})  
extended_bane_brain_sap      = extended_bane_brain_sap      or class({})  
extended_bane_nightmare      = extended_bane_nightmare      or class({})  
extended_bane_nightmare_end  = extended_bane_nightmare_end  or class({}) 
extended_bane_fiends_grip    = extended_bane_fiends_grip    or class({})

-- Enfeeble Debuff
modifier_extended_bane_enfeeble = class({
...
-- brain_sap Debuff
modifier_extended_bane_brain_sap = class({
		IsDebuff                         = function(self) return true                                                                                                           end,
...
-- Nightmare Debuff
modifier_extended_bane_nightmare = class({
		IsDebuff                         = function(self) return true                                                                                                           end,
...
modifier_extended_bane_nightmare_invuln_period = class({
		IsDebuff                         = function(self) return true                                                                                                           end,    
		IsHidden                         = function(self) return true                                                                                                           end,
	})
modifier_extended_bane_nightmare_baleful_visions = class({
		GetBonusVisionPercentage         = function(self) return self.visionreduction                                                                                           end,    
	})
-- Fiends Grip Debuff
modifier_extended_bane_fiends_grip = class({
		IsDebuff                         = function(self) return true                                                                                                           end,
...
LinkLuaModifier("modifier_extended_bane_enfeeble",                  "hero/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_bane_brain_sap",                 "hero/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_bane_nightmare",                 "hero/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_bane_nightmare_invuln_period",   "hero/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_bane_nightmare_baleful_visions", "hero/hero_bane", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_extended_bane_fiends_grip",               "hero/hero_bane", LUA_MODIFIER_MOTION_NONE)
...
function extended_bane_fiends_grip:  GetChannelAnimation() return ACT_DOTA_CHANNEL_ABILITY_4 or ACT_DOTA_CHANNEL_ABILITY_5 end
function extended_bane_enfeeble:     IsHiddenWhenStolen()  return false                                                    end
function extended_bane_brain_sap:    IsHiddenWhenStolen()  return false                                                    end
function extended_bane_nightmare:    IsHiddenWhenStolen()  return false                                                    end
function extended_bane_nightmare_end:IsHiddenWhenStolen()  return false                                                    end
function extended_bane_fiends_grip:  IsHiddenWhenStolen()  return false                                                    end

-- Enfeeble Spell Cast
function extended_bane_enfeeble:OnSpellStart() 
	if not IsServer() then return end
	local target,caster = findtarget(self)
	if target:TriggerSpellAbsorb(self) then return end
	local enfeeble_duration = getkvValues(self,"enfeeble_duration")  
	target:AddNewModifier(caster, self, "modifier_extended_bane_enfeeble", {duration = enfeeble_duration})
...
function extended_bane_brain_sap:OnSpellStart()
	if not IsServer() then return end
	local target,caster = findtarget(self)
	...
	target:AddNewModifier(caster, self, "modifier_extended_bane_brain_sap", {duration = (sapduration+talentdurationbonus)}) -- debuff enemy
...
function extended_bane_brain_sap:GetCastRange(l,t)
...
function extended_bane_nightmare:OnUpgrade()
	if self:GetCaster():HasAbility("extended_bane_nightmare_end") and self:GetCaster():FindAbilityByName("extended_bane_nightmare_end"):GetLevel() ~= 1 then --do we have nightmareend? is nightmare end skilled?
		self:GetCaster():FindAbilityByName("extended_bane_nightmare_end"):SetLevel(1) -- skill it if both true
	end
end

function extended_bane_nightmare:GetAssociatedSecondaryAbilities()
	return "extended_bane_nightmare_end" 
end 

function extended_bane_nightmare:OnSpellStart()
	if not IsServer() then return end
	...
	target:AddNewModifier(caster, self, "modifier_extended_bane_nightmare",               {duration = nightmareduration}) -- im schleep
	target:AddNewModifier(caster, self, "modifier_extended_bane_nightmare_invuln_period", {duration = invulnduration})
...
function extended_bane_nightmare_end:OnSpellStart()
	if not IsServer() then return end
	local target,caster = findtarget(self)
	EmitSoundOn("Hero_Bane.Nightmare.End", caster)
	if target:HasModifier("modifier_extended_bane_nightmare_invuln_period") then -- We can't debuff an immune target! so remove this first
		target:RemoveModifierByName("modifier_extended_bane_nightmare_invuln_period")
	end    
	if target:HasModifier("modifier_extended_bane_nightmare") then  --do we have it? remove it
		target:RemoveModifierByName("modifier_extended_bane_nightmare")
	end
end

function extended_bane_nightmare_end:GetAssociatedPrimaryAbilities()
	return "extended_bane_nightmare" end

-- Fiends Grip Spell Cast

function extended_bane_fiends_grip:GetChannelTime()
...
function extended_bane_fiends_grip:GetCooldown(nLevel)
...
function extended_bane_fiends_grip:OnSpellStart()  
	if not IsServer() then return end
	...
	self.fiendtarget:AddNewModifier(self.fiendcaster, self, "modifier_extended_bane_fiends_grip", {duration = fiends_grip_duration+talentchannelbonus,propogated = 0, original_target = 1}) -- we are the original target, it has not spread
...
function extended_bane_fiends_grip:OnChannelFinish(bInterrupted) 
	if not IsServer() then return end  
	for k,v in pairs(self.fiendgriptable) do
		...
		if ((self.fiendgriptable[k]:HasModifier("modifier_extended_bane_fiends_grip")) and self.fiendgriptable[k]:FindModifierByName("modifier_extended_bane_fiends_grip").propogated == 0) then
			self.fiendgriptable[k]:FindModifierByName("modifier_extended_bane_fiends_grip"):Destroy() -- if our target has been gripped, and it has not been spread by itself, destroy it (which will reinvoke and set propogated to 1)
		end
	end
end

-- We only care about channel operations when he has scepter
function extended_bane_fiends_grip:OnChannelThink()
	if not IsServer() then return end
	if self:GetCaster():HasScepter() then
		...
		for _,v in pairs(enemies_to_check) do
			...
			local isgripped = v:HasModifier("modifier_extended_bane_fiends_grip")
			...
			if angle <= ( vision_cone / 2 ) and v:CanEntityBeSeenByMyTeam(self.fiendcaster) and v~=self.fiendtarget and not isgripped and not v:IsMagicImmune() then
				v:AddNewModifier(self.fiendcaster, self, "modifier_extended_bane_fiends_grip", {duration = ((fiends_grip_duration+talentchannelbonus)-(GameRules:GetGameTime()-self:GetChannelStartTime())),propogated = 0})
...
function modifier_extended_bane_enfeeble:DeclareFunctions()
	local funcs = {
...
function modifier_extended_bane_enfeeble:OnCreated()
...
function modifier_extended_bane_brain_sap:DeclareFunctions()
	local funcs = {
...
function modifier_extended_bane_nightmare:DeclareFunctions()
	local funcs = {
...
function modifier_extended_bane_nightmare:OnCreated()
...
function modifier_extended_bane_nightmare:CheckState()
	return{
...
function modifier_extended_bane_nightmare:OnTakeDamage(t)
	if not IsServer() then return end
...
function modifier_extended_bane_nightmare:OnAttackStart(t)
	if not IsServer() then return end
	if t.target == self:GetParent() then -- am i the target of an attack?
		...
		t.attacker:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_extended_bane_nightmare", {duration = nightmare_duration}) -- Spread Nightmare
		t.attacker:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_extended_bane_nightmare_invuln_period", {duration = nightmare_invuln_duration}) -- Apply invuln, idk if firetoad wanted allies to gain the talent so i'm leaving original functionality
...
function modifier_extended_bane_nightmare:OnIntervalThink()
	if not IsServer() then return end
...
function modifier_extended_bane_nightmare:OnDestroy()
	if not IsServer() then return end
	if self:GetCaster():GetTeamNumber() ~= self:GetParent():GetTeamNumber() then -- when nightmare is destroyed, are we an enemy?
		if not self:GetParent():IsMagicImmune() then  -- are we magic immune if we're an enemy?
			...
			self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_extended_bane_nightmare_baleful_visions", {duration = nightmare_baleful_visions_duration+talentdurationbonus}) -- blind them
		end
	end
	if self:GetParent():HasModifier("modifier_extended_bane_nightmare_invuln_period") then -- If we don't do this, clever players will only skill one point in nightmare and take the 6sec invuln talent
		self:GetParent():RemoveModifierByName("modifier_extended_bane_nightmare_invuln_period")
	end      
	EmitSoundOn("Hero_Bane.Nightmare.End", self:GetParent())
end

-- Nightmare Invulnerability
function modifier_extended_bane_nightmare_invuln_period:DeclareFunctions()
	local funcs = {
...
function modifier_extended_bane_nightmare_invuln_period:CheckState()
	return{[MODIFIER_STATE_INVULNERABLE] = true, [MODIFIER_STATE_NO_HEALTH_BAR] = true}
end

-- Nightmare Baleful Visions

function modifier_extended_bane_nightmare_baleful_visions:DeclareFunctions()
	local funcs = {
		MODIFIER_PROPERTY_BONUS_VISION_PERCENTAGE,
	}
	return funcs
...
function modifier_extended_bane_nightmare_baleful_visions:OnCreated()
	self:StartIntervalThink(0)
end

function modifier_extended_bane_nightmare_baleful_visions:OnIntervalThink()
...
function modifier_extended_bane_fiends_grip:DeclareFunctions()
	local funcs = {
		MODIFIER_PROPERTY_OVERRIDE_ANIMATION,
	}
	return funcs  
...
function modifier_extended_bane_fiends_grip:OnCreated(var)
	if not IsServer() then return end 
...
function modifier_extended_bane_fiends_grip:OnRefresh(var)
	if not IsServer() then return end   
	self:GetParent():Interrupt()    
	self:OnIntervalThink()  
	self:StartIntervalThink(1)
...
function modifier_extended_bane_fiends_grip:OnIntervalThink()
	if not IsServer() then return end 
...
function modifier_extended_bane_fiends_grip:CheckState()
	local state = {
		[MODIFIER_STATE_STUNNED] = true,
	}
	return state
...
function modifier_extended_bane_fiends_grip:OnDestroy()  
	if not IsServer() then return end   
	...
	if not self:GetParent():IsMagicImmune() and self.propogated == 0 then -- Are we magic immune somehow(omniknight)? if not, have we already retriggered once?
		...
		self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_extended_bane_fiends_grip", {duration = fiends_grip_linger_duration, propogated = 1})   
	end
end
