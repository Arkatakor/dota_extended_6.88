...
function PickupBountyRune(item, unit)
	...
	if unit:FindAbilityByName("extended_alchemist_goblins_greed") and unit:FindAbilityByName("extended_alchemist_goblins_greed"):GetLevel() > 0 then
		current_bounty = current_bounty * unit:FindAbilityByName("extended_alchemist_goblins_greed"):GetSpecialValueFor("bounty_multiplier")

		-- #7 Talent: Doubles gold from bounty runes
		if unit:HasTalent("special_bonus_extended_alchemist_7") then
			current_bounty = current_bounty * unit:FindTalentValue("special_bonus_extended_alchemist_7")
		end
	end
