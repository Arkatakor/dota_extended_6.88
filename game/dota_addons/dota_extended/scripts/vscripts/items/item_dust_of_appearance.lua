...
LinkLuaModifier( "modifier_extended_dust_of_appearance", "items/item_dust_of_appearance.lua", LUA_MODIFIER_MOTION_NONE )	-- Dust reveal condition
-----------------------------------------------------------------------------------------------------------
--	Item Definition 
-----------------------------------------------------------------------------------------------------------

if item_extended_dust_of_appearance == nil then item_extended_dust_of_appearance = class({}) end
function item_extended_dust_of_appearance:GetBehavior() return DOTA_ABILITY_BEHAVIOR_IMMEDIATE + DOTA_ABILITY_BEHAVIOR_NO_TARGET end

function item_extended_dust_of_appearance:OnSpellStart()
	...
	for _,unit in pairs(targets) do
		...
		unit:AddNewModifier(caster, self, "modifier_extended_dust_of_appearance", {duration = duration})
	end
...
if modifier_extended_dust_of_appearance == nil then modifier_extended_dust_of_appearance = class({}) end
function modifier_extended_dust_of_appearance:IsDebuff() return true end
function modifier_extended_dust_of_appearance:IsHidden() return false end
function modifier_extended_dust_of_appearance:IsPurgable() return true end

function modifier_extended_dust_of_appearance:DeclareFunctions()	
	local decFuncs = {	MODIFIER_PROPERTY_PROVIDES_FOW_POSITION,
						MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,	}
	return decFuncs	
end

function modifier_extended_dust_of_appearance:GetEffectName()
	return "particles/items2_fx/true_sight_debuff.vpcf" end
	
function modifier_extended_dust_of_appearance:GetEffectAttachType()
	return PATTACH_OVERHEAD_FOLLOW end

function modifier_extended_dust_of_appearance:GetPriority()
	return MODIFIER_PRIORITY_SUPER_ULTRA end

function modifier_extended_dust_of_appearance:CheckState()
	return {[MODIFIER_STATE_INVISIBLE] = false,}
end

function modifier_extended_dust_of_appearance:GetModifierProvidesFOWVision()
...
function modifier_extended_dust_of_appearance:GetModifierMoveSpeedBonus_Percentage()
	local parent = self:GetParent()
